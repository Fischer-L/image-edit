...
var imgCanvas = {
   ...
   *                               source by `setSource`
   */
  init: function init(outputImg) {
    this._outputImg = outputImg;
    this._cvs = document.createElement("canvas");
    this._ctx = this._cvs.getContext("2d");
    this._cssFilters = new _map2.default();
  ...
  setSource: function setSource(source) {
    var srcWidth = source.img.naturalWidth;
    var srcHeight = source.img.naturalHeight;
  ...
  toBlob: function toBlob() {
    ...
    return new _promise2.default(function (resolve) {
      if (!_this._source || _this._source.isCORS) {
        resolve(null);
        return;
      }
      window.requestIdleCallback(function () {
        var filter = _this._outputImg.style.filter;
        var outputCtx = document.createElement("canvas").getContext("2d");
        outputCtx.canvas.width = _this._cvs.width;
        outputCtx.canvas.height = _this._cvs.height;
        if (filter) {
          outputCtx.filter = filter;
        }
        outputCtx.drawImage(_this._cvs, 0, 0);
        outputCtx.canvas.toBlob(function (blob) {
          return resolve(blob);
        });
      });
    });
  },
  _drawCanvas: function _drawCanvas(img, dimension, filter) {
    var srcX = dimension.srcX,
        srcY = dimension.srcY,
        srcWidth = dimension.srcWidth,
        srcHeight = dimension.srcHeight,
        dstWidth = dimension.dstWidth,
        dstHeight = dimension.dstHeight;
  ...
  _drawToImg: function _drawToImg(w, h) {
    ...
    this._outputImg.src = this._source.isCORS ? this._source.img.src : this._cvs.toDataURL();
    this._applyCSSFilter();
  },
  _applyCSSFilter: function _applyCSSFilter() {
    ...
    if (this._cssFilters.size) {
      var rules = [];
      this._cssFilters.forEach(function (lv, effect) {
        rules += effect + "(" + lv + "%) ";
      });
   ...
   */
  applyGrayScale: function applyGrayScale(lv) {
    if (!this._source) {
      return;
    }
   ...
   * @param lv {Number} the effect level from 0 ~ 1
   */
  applySepia: function applySepia(lv) {
    if (!this._source) {
      return;
    }
   ...
   *                           30% ~ 60% on the y axis.
   */
  cropImg: function cropImg(range) {
    if (!this._source) {
      return;
    }
    ...
    if (this._source.isCORS) {
      var _range$map = range.map(function (bound) {
        return bound * 100 + "%";
      }),
          _range$map2 = (0, _slicedToArray3.default)(_range$map, 4),
          _lowerX = _range$map2[0],
          _upperX = _range$map2[1],
          _lowerY = _range$map2[2],
          _upperY = _range$map2[3];

      this._outputImg.style.clipPath = "polygon(" + _lowerX + " " + _lowerY + ", " + _upperX + " " + _lowerY + ", " + _upperX + " " + _upperY + ", " + _lowerX + " " + _upperY + ")";
      return;
    }
    ...
    var totalWidth = this._outputImg.naturalWidth;
    var totalHeight = this._outputImg.naturalHeight;
    var dimension = {
   ...
   */
  applyDOF: function () {
    var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(focusRange) {
      var _this2 = this;
      ...
      return _regenerator2.default.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              imgData = this._ctx.getImageData(0, 0, this._cvs.width, this._cvs.height);
              _context.next = 5;
              return new _promise2.default(function (resolve) {
                _this2._convolutionWorker.onmessage = function (e) {
                  _this2._convolutionWorker.onmessage = null;
                  resolve(e.data);
                };
                _this2._convolutionWorker.postMessage([imgData, focusRange]);
              });

            case 5:
              newImgData = _context.sent;
...
var imgLoader = {
   ...
   */
  init: function init(imgEditor, listener) {
    var _this = this;
    ...
    imgEditor.addEventListener("click", function (e) {
      return _this._handleClick(e);
    });

    this._inputFile.addEventListener("change", function (e) {
      _this._handleInputFromFile(e);
    });

    this._imgDropArea.addEventListener("dragover", function (e) {
      e.stopPropagation();
      e.preventDefault();
    });
    this._imgDropArea.addEventListener("drop", function (e) {
      e.stopPropagation();
      e.preventDefault();
      _this._handleInputFromFile(e);
    });
  ...
  _handleClick: function _handleClick(e) {
    switch (e.target.id) {
      ...
      case "url-input-btn":
        var url = prompt("Please input the image url");
        this._handleInputFromURL(url);
        break;
    }
  },
  _createImg: function _createImg(url, isCORS) {
    var _this2 = this;
    ...
    img.onload = function () {
      ...
      _this2._listener.onImgLoaded({ img: img, isCORS: isCORS });
    };
    img.src = url;
  },
  _handleInputFromURL: function _handleInputFromURL(url) {
    // Check https or http or base64 urls
    var reg = /https:\/\/|http:\/\/|data:image\/.*;base64/;
  ...
  _handleInputFromFile: function _handleInputFromFile(e) {
    var files = e.target.files || e.dataTransfer.files;
    var file = files ? files[0] : null; // Currently only support a single file first
  ...
  _revokeImgObjURL: function _revokeImgObjURL() {
    if (this._imgObjURL) {
...
var _imgLoader = __webpack_require__(/*! ./imgLoader */ "./src/imgLoader.js");

var _imgLoader2 = _interopRequireDefault(_imgLoader);

var _imgCanvas = __webpack_require__(/*! ./imgCanvas */ "./src/imgCanvas.js");

var _imgCanvas2 = _interopRequireDefault(_imgCanvas);

var _loading = __webpack_require__(/*! ./loading.svg */ "./src/loading.svg");
...
var imgEditor = {
  init: function init(imgCanvas, imgLoader) {
    var _this = this;
    ...
    this._controlPanel.addEventListener("click", function (e) {
      return _this.onClick(e);
    });
    this._controlPanel.addEventListener("change", function (e) {
      return _this.onChange(e);
    });
    this._controlPanel.addEventListener("mousedown", function (e) {
  ...
  _resetControlPanel: function _resetControlPanel() {
    var inputs = this._controlPanel.querySelectorAll(".control-input");
    ...
    var dofControl = this._controlPanel.querySelector("#effect-dof").parentNode;
    var downloadControl = this._controlPanel.querySelector("#download").parentNode;
    if (this._source && this._source.isCORS) {
    ...
    var urlBtn = document.querySelector("#url-input-btn");
    if (true && urlBtn) {
      urlBtn.remove();
    }
  },
  onClick: function onClick(e) {
    ...
    switch (e.target.id) {
      case "effect-crop":
        this._selectImgRange(function (range) {
          return _this2.cropImg(range);
        });
        return;

      case "effect-dof":
        this._selectImgRange(function (range) {
          return _this2.applyDOF(range);
        });
        return;
  ...
  onImgLoaded: function onImgLoaded(source) {
    this._source = source;
      ...
      return _regenerator2.default.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 5:
              link = this._controlPanel.querySelector("#download-link");
  ...
  _applyFilterChange: function _applyFilterChange(input) {
    switch (input.id) {
      case "effect-sepia":
  ...
  _trackFilterChange: function _trackFilterChange(input) {
    var _this3 = this;
    ...
    this._onUserDragInput = function () {
      return _this3._applyFilterChange(input);
    };
    input.addEventListener("mousemove", this._onUserDragInput);

    this._stopTrackFilterChange = function () {
      input.removeEventListener("mousemove", _this3._onUserDragInput);
      input.removeEventListener("mouseup", _this3._stopTrackFilterChange);
      _this3._onUserDragInput = _this3._stopTrackFilterChange = null;
    };
    input.addEventListener("mouseup", this._stopTrackFilterChange);
  },
  _showCoverImg: function _showCoverImg(coverImg, coverArea) {
    ...
    var className = "img-panel--center ";
    if (coverArea.className) {
      className += coverArea.className;
    }
    this._coverArea.className = className;
  },
  _hideCoverImg: function _hideCoverImg() {
    this._coverArea.className = "no-display";
  },
  _selectImgRange: function () {
    var _ref2 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee2(onSelect) {
      var range;
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            ...
            case 2:
              ...
              _context2.next = 5;
              return this._selectRangePromise;
    ...
    function _selectImgRange(_x) {
      return _ref2.apply(this, arguments);
    }

    return _selectImgRange;
  }(),
  _trackImgSelectRange: function () {
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 2:
              _context3.next = 4;
              return new _promise2.default(function (resolve) {
                var totalWidth = _this4._outputImg.width;
                var totalHeight = _this4._outputImg.height;
                _this4._pickSelectRangeOrign = function (e) {
                  _this4._coverArea.removeEventListener("mousedown", _this4._pickSelectRangeOrign);
                  resolve([e.offsetX, e.offsetY, totalWidth, totalHeight]);
                };
                _this4._coverArea.addEventListener("mousedown", _this4._pickSelectRangeOrign);
                _this4._showCoverImg({
                  src: _this4._outputImg.src,
                  clipPath: "polygon(0% 0%, 0% 0%, 0% 0%, 0% 0%)",
                  width: totalWidth,
                  height: totalHeight
                }, {
                  clipPath: _this4._outputImg.style.clipPath,
                  width: totalWidth,
                  height: totalHeight
                });
              });
            ...
            case 4:
              pos = _context3.sent;
              return _context3.abrupt("return", this._detectRangeOnMousemove.apply(this, (0, _toConsumableArray3.default)(pos)));
    ...
    function _trackImgSelectRange() {
      return _ref3.apply(this, arguments);
    }

    return _trackImgSelectRange;
  }(),
   ...
   */
  _detectRangeOnMousemove: function _detectRangeOnMousemove(originX, originY, totalWidth, totalHeight) {
    var _this5 = this;
    ...
    return new _promise2.default(function (resolve) {
      _this5._drawImgSelectRange = function (e) {
        var range = _this5._computeSelectRange(totalWidth, totalHeight, [originX, originY], [e.offsetX, e.offsetY]);

        var _range$map = range.map(function (bound) {
          return bound * 100 + "%";
        }),
            _range$map2 = (0, _slicedToArray3.default)(_range$map, 4),
            lowerX = _range$map2[0],
            upperX = _range$map2[1],
            lowerY = _range$map2[2],
            upperY = _range$map2[3];

        _this5._coverImg.style.clipPath = "polygon(" + lowerX + " " + lowerY + ", " + upperX + " " + lowerY + ", " + upperX + " " + upperY + ", " + lowerX + " " + upperY + ")";
      };
      _this5._coverArea.addEventListener("mousemove", _this5._drawImgSelectRange);

      _this5._stopTrackImgSelectRange = function (e) {
        _this5._coverArea.removeEventListener("mousemove", _this5._drawImgSelectRange);
        _this5._coverArea.removeEventListener("mouseup", _this5._stopTrackImgSelectRange);
        _this5._pickSelectRangeOrign = _this5._drawImgSelectRange = _this5._stopTrackImgSelectRange = null;

        var range = _this5._computeSelectRange(totalWidth, totalHeight, [originX, originY], [e.offsetX, e.offsetY]);
        // Alwasy hide the cover area in the end so as to keep the mouse position correct
        _this5._hideCoverImg();
        resolve(range);
      };
      _this5._coverArea.addEventListener("mouseup", _this5._stopTrackImgSelectRange);
    });
  },
  _computeSelectRange: function _computeSelectRange(totalWidth, totalHeight, startPos, endPos) {
    // `startX` is the start point of the slection range bound (0 ~ 1) on the x axis.
    // `startY` is for the y axis.
    var startX = startPos[0] / totalWidth;
    var startY = startPos[1] / totalHeight;
    var endX = endPos[0] / totalWidth;
    var endY = endPos[1] / totalHeight;
    var lowerX = Math.min(startX, endX);
    var upperX = Math.max(startX, endX);
    var lowerY = Math.min(startY, endY);
    var upperY = Math.max(startY, endY);
  ...
  cropImg: function cropImg(range) {
    ...
    window.requestAnimationFrame(function () {
      return _this6._imgCanvas.cropImg(range);
    });
  },
  applyDOF: function applyDOF(range) {
    var _this7 = this;

    window.requestAnimationFrame((0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee4() {
      return _regenerator2.default.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              _this7._showCoverImg({
                src: _loading2.default
              }, {
                className: "loading-img",
                width: _this7._outputImg.width,
                height: _this7._outputImg.height
              });
              _context4.next = 3;
              return _this7._imgCanvas.applyDOF(range);

            case 3:
              _this7._hideCoverImg();
  ...
  applySepia: function applySepia(lv) {
    ...
    window.requestAnimationFrame(function () {
      return _this8._imgCanvas.applySepia(lv);
    });
  },
  applyGrayScale: function applyGrayScale(lv) {
    var _this9 = this;
    ...
    window.requestAnimationFrame(function () {
      return _this9._imgCanvas.applyGrayScale(lv);
    });
  }
};

imgEditor.init(_imgCanvas2.default, _imgLoader2.default);
